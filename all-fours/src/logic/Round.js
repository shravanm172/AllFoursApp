...
export class Round {
  ...
  async playRound() {
    ...
    // Show updated hands after begging phase
    this.io.showPlayerHands(this.players);
  ...
  async playTrick(trick, trickNumber) {
    while (!trick.isComplete()) {
      ...
      try {
        ...
        console.log("🎯 BEFORE showTrickState - playedCards:", trick.playedCards.length, trick.playedCards.map(entry => `${entry.player.getName()}: ${entry.card.toString()}`));
        this.io.showTrickState([...trick.playedCards]);
        console.log("🎯 AFTER showTrickState - trick complete?", trick.isComplete());
